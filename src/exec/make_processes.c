...
/*   Updated: 2024/04/06 05:06:04 by saraki           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	make_processes(t_tokenlst *token_node, t_pipelst *pipe_node)
{
	...
	while (pipe_node->data != NULL)
	{
		if (pipe_node->prev->data == NULL)
			err = make_process(token_node, pipe_node, do_first_process);
		else if (pipe_node->next->data == NULL)
...
static int	init_pipeline(t_pipelst *pipe_node)
{
	...
	while (pipe_node->data != NULL)
	{
		pipe = (t_pipex *) pipe_node->data;
		if (pipe_node->next->data != NULL)
		{
			next_pipe = pipe_node->next->data;
