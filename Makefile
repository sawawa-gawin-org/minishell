...
LFLAGS := -L$(LIB_EXPORT_DIR) -ltokens -lexec -lbuiltin -lpath -lenv -ldbllst -lft -lcommon -lreadline
DLFLAGS := -L$(LIB_EXPORT_DIR) -ltokens_debug -lexec_debug -lbuiltin_debug -lpath_debug -lenv_debug -ldbllst_debug -lft_debug -lcommon_debug -lreadline
DFLAGS := -fdiagnostics-color=always -g3 -fsanitize=address
IFLAGS := -I$(HEADER_DIR)

DEPENDENCY := libft dbllst exec tokens env common builtin path
DEPENDENCY_LIB := libft.a libdbllst.a libexec.a libtokens.a libenv.a libcommon.a libbuiltin.a libpath.a
